...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            # NEW: Generalized Feature-Level Token Orthogonality
            if c_outputs is not None and c_outputs.shape[2] > 1:
                # final_tokens shape: [B, C, D] (where C is usually 20 for VOC)
                final_tokens = c_outputs[-1]
                
                # Normalize tokens to easily compute cosine similarity via dot product
                final_tokens_norm = F.normalize(final_tokens, dim=-1)
                
                # Compute pairwise cosine similarity matrix: [B, C, C]
                sim_matrix = torch.bmm(final_tokens_norm, final_tokens_norm.transpose(1, 2))
                
                # We want them to be orthogonal (similarity <= 0). Penalize > 0.
                penalty_matrix = torch.clamp(sim_matrix, min=0.0)
                
                # Create a mask for pairs where BOTH classes are present in the image
                # targets shape: [B, C] -> target_pairs shape: [B, C, C]
                target_pairs = targets.unsqueeze(2) * targets.unsqueeze(1)
                
                # Remove the diagonal (we don't want to penalize a class's similarity with itself)
                B, C, _ = sim_matrix.shape
                diag_mask = 1.0 - torch.eye(C, device=device).unsqueeze(0)
                
                # Final valid mask: off-diagonal elements where both classes exist
                valid_mask = target_pairs * diag_mask
                
                # Compute masked mean (1e-4 prevents FP16 underflow!)
                token_penalty = (penalty_matrix * valid_mask).sum() / (valid_mask.sum() + 1e-4)
                
                metric_logger.update(sep_loss=token_penalty.item())
                
                # Apply warmup and weight
