...
# Add the Base model to the __all__ list
__all__.append('deit_base_MCTformerPlus')

@register_model
def deit_base_MCTformerPlus(pretrained=False, **kwargs):
    # Base configuration: 768 embedding dimension and 12 attention heads
    model = MCTformerPlus(
...
# Add the ViT-Base model to the __all__ list
__all__.append('vit_base_MCTformerPlus')

@register_model
def vit_base_MCTformerPlus(pretrained=False, **kwargs):
    # ViT-Base configuration: 768 embedding dimension and 12 attention heads
    model = MCTformerPlus(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
...
# Adding the DINO model to the __all__ list
__all__.append('dino_base_MCTformerPlus')

@register_model
def dino_base_MCTformerPlus(pretrained=False, **kwargs):
    # Physically identical to ViT-Base and DeiT-Base
    model = MCTformerPlus(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    ...
    if pretrained:
        # The official DINO ViT-Base checkpoint
        checkpoint = torch.hub.load_state_dict_from_url(
