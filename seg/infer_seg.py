...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        # Fetch all three metrics gracefully from the updated Evaluator
        IoU, mIoU = seg_evaluator.Mean_Intersection_over_Union()
        Dice, mDice = seg_evaluator.Mean_Dice()
        HD95, mHD95 = seg_evaluator.Mean_HD95()

        # Print and Save formatting
        filename = os.path.join(args.save_path, 'final_voc_results.txt')
        with open(filename, 'w') as f:
            import time
            ...
            for k in range(args.num_classes):
                print(f"{classes[k]:<15} | {IoU[k]*100:<10.3f} | {Dice[k]*100:<10.3f} | {HD95[k]:<10.3f}")
                f.write(f"{classes[k]:<15} | {IoU[k]*100:<10.3f} | {Dice[k]*100:<10.3f} | {HD95[k]:<10.3f}\n")
                
            f.write("=" * 60 + "\n")
            f.write(f"{'MEAN OVERALL':<15} | {mIoU*100:<10.3f} | {mDice*100:<10.3f} | {mHD95:<10.3f}\n")
            
            print("=" * 55)
            print(f"{'MEAN OVERALL':<15} | {mIoU*100:<10.3f} | {mDice*100:<10.3f} | {mHD95:<10.3f}\n")
            
        print(f"---> Detailed results saved successfully to {filename}")